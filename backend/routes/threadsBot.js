...
async function groqCaption(asset) {
  const sys = "Ты SMM-копирайтер. Короткий цепляющий пост для Threads на русском о БЕСПЛАТНОЙ платформе rbhood ai — ИИ-анализ графиков (форекс, крипто, акции) за 7 секунд. Живой тон, без воды. 2-4 строки + 3-5 хэштегов. Обязательно: БЕСПЛАТНО и ссылка ai.rbhood.kz.";
  try {
    const r = await fetch("https://api.groq.com/openai/v1/chat/completions", {
      method: "POST", headers: { "Content-Type": "application/json", Authorization: `Bearer ${GROQ}` },
      body: JSON.stringify({ model: "llama-3.3-70b-versatile", max_tokens: 300, temperature: 1.1, messages: [{ role: "system", content: sys }, { role: "user", content: `Актив: ${asset}. Другой вариант поста.` }] }),
    });
    const j = await r.json(); const t = (j.choices?.[0]?.message?.content || "").trim(); if (t) return t;
  } catch { /* ignore */ }
  return `Бесплатный ИИ-анализ ${asset} за 7 секунд 🚀 ai.rbhood.kz #трейдинг #крипто`;
}
